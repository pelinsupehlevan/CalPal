...
class AddFoodFragment : Fragment(R.layout.fragment_add_food){
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        binding.addFoodButton.setOnClickListener{
            if(validate()){
                saveFoodData()
                view.findNavController().navigate(R.id.action_addFoodFragment_to_mainFragment)
            }else{
    ...
    private fun validate() : Boolean{
        if(binding.setFoodName.text.isEmpty()){
            return false
        }
        if(binding.setCalorie.text.isEmpty()){
            return false
        }
        return true
    }
    ...
    private fun saveFoodData(){
        val FoodName = binding.setFoodName.text.toString()
        val FoodCalorie = binding.setCalorie.text.toString().toInt()
